...
prj_rd_oldroad_gs <- readRDS(file.path(project_file_path,"Data", "Project Roads",
...
prj_rd_oldroad_gs   <- gBuffer(prj_rd_oldroad_gs,    width = .001/111.12, byid=T)

## Dissolve
#prj_rd_r78ab$one <- 1
#prj_rd_r78ab <- raster::aggregate(prj_rd_r78ab, by="one")

prj_rd_oldroad_gs$one <- 1
prj_rd_oldroad_gs<- raster::aggregate(prj_rd_oldroad_gs, by="one")

# Reproject --------------------------------------------------------------------
grid          <- spTransform(grid,          CRS(UTM_IRQ))
#prj_rd_r78ab  <- spTransform(prj_rd_r78ab, CRS(UTM_IRQ))
prj_rd_oldroad_gs    <- spTransform(prj_rd_oldroad_gs, CRS(UTM_IRQ))

# Calculate Distance -----------------------------------------------------------
#grid$dist_r78ab_road_km <- gDistance_chunks(grid, prj_rd_r78ab, 5000, 1) / 1000 # convert from meters to km
grid$dist_oldroad_km   <- gDistance_chunks(grid, prj_rd_oldroad_gs, 5000, 1) / 1000 # convert from meters to km
