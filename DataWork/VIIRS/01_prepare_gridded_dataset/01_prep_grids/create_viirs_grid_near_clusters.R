...
# other variables to VIIRs level. Creates grid for clusters
...
# 1. Load Data -----------------------------------------------------------------
#### GADM
iraq <- readRDS(file.path(project_file_path, "Data", "GADM", "RawData", 
                          "gadm36_IRQ_0_sp.rds"))

#### Project urban_clusters
clusters <- readRDS(file.path(data_file_path, "Globcover", "FinalData", "polygons.Rds"))
#### VIIRS
...
viirs_cf_cvg <- stack(file.path(project_file_path, "Data", "VIIRS", "RawData", 
...
#### Prep urban_clusters shapefile
## Common variables
clusters@data <- clusters@data %>% mutate(id = 1) %>% dplyr::select(id)

## Common CRS
clusters <- clusters %>% spTransform(CRS("+init=epsg:4326"))

## Buffer
# Larger buffer around road in the north
clusters <- gBuffer_chunks(clusters, width=DIST_ROAD/111.12, 100)

## Append
urban_clusters <- clusters

## Dissolve
urban_clusters <- gBuffer(urban_clusters, width=0, byid=T) # cleanup self intersections
urban_clusters <- raster::aggregate(urban_clusters, by = "id")

## Simplify
urban_clusters <- gSimplify(urban_clusters, tol = .01)

## Add Variable
urban_clusters$in_area <- 1
...
# Mask urban_clusters to country to get study area: in both urban_clusters and country
urban_clusters <- raster::intersect(urban_clusters, iraq)
...
r_OVER_urban_clusters   <- over_chunks(r_tmp_coords, urban_clusters, "sum", 10000)

cell_in_analysis <- (r_OVER_urban_clusters$in_area %in% 1) 
...
iraq_grid_viirs <- data.frame(avg_rad_df=avg_rad_df, 
                              ...
                              band=band)
# Add in coordinates
iraq_grid_viirs <- merge(iraq_grid_viirs, viirs_coords_in_iraq, by="id") ##check error

head(iraq_grid_viirs)

head(viirs_coords_in_iraq)
...
# Export ------------------------------------------------------------------
OUT_PATH <- file.path(project_file_path, "Data", "VIIRS", "FinalData", 
                      "near_clusters", "Separate Files Per Variable")
