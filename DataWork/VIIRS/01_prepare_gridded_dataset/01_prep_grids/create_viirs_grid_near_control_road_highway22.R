...
####Project Roads
prj_rd_highway2 <- readRDS(file.path(project_file_path,"Data", "Project Roads",
                                      "Control_Roads","FinalData","highway2.Rds"))
...
## Common variables
prj_rd_highway2@data <- prj_rd_highway2@data %>% mutate(id = 1) %>% dplyr::select(id)

## Common CRS
prj_rd_highway2 <- prj_rd_highway2 %>% spTransform(CRS("+init=epsg:4326"))

## Buffer
# Larger buffer around road in the north
prj_rd_highway2 <- gBuffer_chunks(prj_rd_highway2, width=DIST_ROAD/111.12, 100)

## Append
roads <- prj_rd_highway2
...
# Determine if cell should be in analysis: near a road and in the country.
...
# Coordinates ------------------------------------------------------------------
viirs_coords_in_iraq <- r_tmp_coords[cell_in_analysis,] %>% 
  coordinates %>% 
  as.data.frame
...
extract_raster_value_in_country <- function(band_num, raster_file_path, 
                                            cell_in_analysis, var_name){
...
saveRDS(iraq_grid_viirs_nolatlon, file=file.path(OUT_PATH, "iraq_grid_panel_viirs_highway2.Rds"))
rm(iraq_grid_viirs_nolatlon)

# 2. Blank Panel
iraq_grid_viirs_blankpanel <- subset(iraq_grid_viirs, select=c(id,month,year, lat, lon))
saveRDS(iraq_grid_viirs_blankpanel, file=file.path(OUT_PATH, "iraq_grid_panel_blank_highway2.Rds"))
...
saveRDS(iraq_grid_viirs_blank, file=file.path(OUT_PATH, "iraq_grid_blank_highway2.Rds"))
