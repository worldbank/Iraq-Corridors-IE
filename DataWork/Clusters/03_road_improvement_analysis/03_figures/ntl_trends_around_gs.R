...
# 1.Subset Data -----------------------------------------------------------
vars <- c("month", "year", "viirs_mean", "dist_gs_km")
cluster <- cluster[vars]

## By Buffer
cluster_5km <- cluster[cluster$dist_gs_km <= 5,]
cluster_5km$buffer <-"5km"

cluster_10km <- cluster[cluster$dist_gs_km <= 10,]
...
cluster_20km <- cluster[cluster$dist_gs_km <= 20,]
...
ggplot() +
  ...
  theme(plot.title=element_text(hjust=0.5, face="bold"))+
  ggsave(file.path(project_file_path, "Data", "Clusters", "Outputs", 
                   "cluster_radiance_5_10_20km.png"), 
         height=height, width=width) 
